...
- 电话机宽度由视口宽、高共同约束，在 390×720 下为约 304 px；≤360 px 的窄屏额外缩小，在 320×568 下约为 230 px。
- 置物台固定在屏幕底部，硬币增大到 42 px 基准尺寸，并强化黄铜高光、边缘与投影。
- 工具栏、状态条、通话稿纸和回复按键使用独立的手机端间距与字体比例。
- 回复面板左右各保留 42 px，完整落在红色电话亭框架内。
- 电话待机位置整体下移；进入通话后再下沉 30–36 px，使抬起的听筒从消息纸张下方露出。
- 620 px 以下的短屏使用无可见滚动条的紧凑消息纸；纵向内容仍可触摸滚动，横向溢出被裁切。
- 开场改为雨夜交换台叙事，只保留“投币外拨”和“铃响接听”两条操作提示。
...
| 320×568 电话机宽度 | 几乎占满 | 约 230 px |
| 320×568 消息纸可见滚动条 | 水平、垂直均出现 | 均隐藏 |
...
![320×568 极窄屏通话](screenshots-mobile-polish/11-final-compact-call.jpg)

通话状态实测：场景 `scrollTop = 0`；390×720 稿纸范围为 `x=12–378`，回复面板范围为 `x=42–348`；320×568 电话宽度约 230 px，听筒下沿位于消息纸下方约 38 px，且消息纸没有可见滚动条。

### 最终开场

![320×568 叙事开场](screenshots-mobile-polish/12-final-intro.jpg)
